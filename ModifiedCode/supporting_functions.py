...
def buildcov(
        site_1, site_2,
        sig, corr_decay, noise,
        analyse=False, cell_d=None):
    """ Generate spatial covariance matrix
    
    Matern 3/2 or Squared exponential. Compared with Jo towards Matlab function
...
class Field(object):
    """
    Field class with data and grid instances
    (in 2D projection and geographic reference) of the area
    ...
    def setGrid(
            self, lat_lrc=62.379897, lon_lrc=5.699939,
            lat_ulc=62.386661, lon_ulc=5.685864,
            proj='wgs84'):
